...
| 颜色 | 含义 | 行为 |
|---|---|---|
| 绿色 | 当前任务/回合完成 | 爆闪 3 次，常亮 60 秒；新的完成会重新爆闪并刷新计时 |
| 黄色 | 等待权限、工具授权或有副作用操作确认 | 爆闪后常亮，工具开始执行后熄灭 |
| 蓝色 | Agent 等待问题回答、选择或补充信息 | 爆闪后常亮，提交回答后熄灭 |
| 红色 | 模型/API、网络、认证、限流或明确的工具失败 | 爆闪后常亮，恢复活动或提交新消息后熄灭 |
...
3. Windows 点击 **连接 JTX-RGB**，在系统蓝牙选择器中确认设备，再点击 **安装适配器** 写入 AI 工具适配器。
4. 点击四张颜色卡进行手动验证。
